...
**Courses and scoring (S22).** A hider can hide several characters in one course (`?h=…`), add a text hint, a nickname, an expiry and a "no object-name hint" difficulty flag per spot. The seeker is guided to the nearest unfound spot (hot/cold banner colours, arrival toast + vibration), can open the target list to switch, and earns points on each catch: rarity base (100/200/400/800) + up to 600 for speed. Characters greet with a speech bubble when they appear and shout "찾았다!" when caught. The menu lists your in-progress course, your shared courses (re-share) and your total score.
...
| `?c=<cache>` | Open a single hidden character (seek mode) |
| `?h=<course>` | Open a course of several hidden characters; produced by the hide flow's share button |
...
| S22 | Gameplay: multi-spot courses, hider hints/nickname/expiry/difficulty, hot-cold banner, timer + score, speech bubbles, re-share ([ADR-0027](docs/adr/0027-gameplay-hunts-hints-scoring.md)) | `S22: courses, hints, scoring` |
...
- Course play (S22, fake GPS): hide card with hint/nickname/expiry fields, and the target list after catching 1 of 2 with the hot-cold banner: ![](docs/verify/s22-hide-card.png) ![](docs/verify/s22-course-targets.png)
